...
def plot_results(X, model):
    # TODO: MC: sample data within range of the given X array
    ...
    # TODO: MC: validate this random array
    idx_arr = np.random.randint(low=0,
                                high=len(X)-1,
                                size=int(0.01*len(X)), # fixed at 1%
                                dtype=int)

    ### Plot heatmap
    fig_step0_heatmap = plt.figure(figsize=(4.8, 4))
    ...
    # TODO: MC: validate this -- adding more color options
    c_arr = plt.cm.tab20.colors 
    fig_step3_outlier_grouping = plt.figure(figsize=(4, 4))
    plt.scatter(X[:, 0], X[:, 1], c='lightgrey', alpha=0.5)

    for l in np.unique(model.labels):
        if l != -1:
            idx = np.where(model.labels == l)[0]
            plt.scatter(X[idx, 0], X[idx, 1], color=c_arr[l], label='GA ' + str(l))
    ...
    fig_step4_anomaly_isocurves = plt.figure(figsize=(4.8, 4))
    ...
    for l in np.unique(model.labels):
        if l != -1:
            plt.plot([], [], '-o', color=c_arr[l], label='GA ' + str(l))
    ...
    ### Step 5: Scoring
    fig_step5_scoring = plt.figure(figsize=(4.4, 4))
    ...
    return(fig_step0_heatmap,
           fig_step1_xray,
           fig_step2_apex_extraction,
           fig_step3_outlier_grouping,
           fig_step4_anomaly_isocurves,
           fig_step5_scoring)
